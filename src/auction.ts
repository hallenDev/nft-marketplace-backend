            ...
            data.forEach(async function(item) {
                ...
                if(_bids.length > 0) {
                    ...
                    if(_bidderInfo && _bidderInfo.email && _bidderInfo.email_notification) {
                        let _msgContent = getAuctionMailContent(_bidderInfo.username?_bidderInfo.username:_bidderInfo.public_address, _asset.name, _asset.id);
                    ...
                    if(_sellerInfo && _sellerInfo.email && _sellerInfo.email_notification) {
                        let _msgContent = getAuctionEndedMailContent(_sellerInfo.username?_sellerInfo.username:_sellerInfo.public_address, _asset.name, _asset.id);
